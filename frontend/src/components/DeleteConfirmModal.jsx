export default function DeleteConfirmModal({ song, onConfirm, onCancel, deleting }) {
  return (
    ...
    >
      <div
        className="sv-modal-panel rounded-2xl w-full max-w-sm p-6"
      ...
      >
        ...
        <div className="mt-6 flex gap-3 justify-end">
          <button
            onClick={onCancel}
            disabled={deleting}
            className="sv-btn-secondary px-4 py-2 rounded-lg text-sm transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={deleting}
            className="px-4 py-2 rounded-lg text-sm text-white bg-red-700/85 hover:bg-red-600 transition-colors disabled:opacity-50 flex items-center gap-2"
          >
            {deleting && (
